<?php
	...
	$author_name = "Claudia-Nicol Zaitseva";
	//echo $author_name;
	...
	if($weekday_now <= 5){
		...
		if($hour_now >= 18 and $hour_now < 19){
			$part_of_day = "kodutööde tegelemis aeg";
		}
	...
	$random_proverb_et = ["Tee tööd töö ajal, aja juttu jutu ajal.","Enne töö, siis lõbu.","Kuri keel on teravam kui nuga.","Suure naeru järel tuleb ikka nutt.","Targem annab järele."];
	//loendan massiivi (array) liikmeid
	//echo count($random_proverb_et);
	//juhuslik arv
	//echo mt_rand(1, 5);
	//juhuslik element massiivist
	//echo $random_proverb_et[mt_rand(0, count($random_proverb_et) - 1)];
	
	//loeme fotode kataloogi sisu
	$photo_dir = "photos/";
	//$all_files = scandir($photo_dir);
	//uus_massiiv = array_slice(massiv,mis kohast alates);
	...
	//loetlen lubatud failitüübi (jpg  png)
	//	MIME
	...
	if(isset($_POST["comment_submit"])){
		...
		if(empty($comment_error)){
			...
			//määrata andmetüübid:	i - integer (täisarv), d - ecimer (murdarv), s - string (tekst)
...
<body>
	<img src="vp_banner_gs.png" alt="bänner">
	...
	<a href="https://www.tlu.ee">
		<img src="tallinna-ulikool.png" alt="Tallinna Ülikooli Astra õppehoone">
	</a>
	...
	<p>Vanasõna: <?php echo $random_proverb_et[mt_rand(0, count($random_proverb_et) - 1)];  ?>.</p>
	<p>Mina olen Claudia-Nicol Zaitseva, õppin Tallinna Ülikoolis Informaatika ja Digimeedia erialal.<p>
